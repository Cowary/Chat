...
public class ServerChat implements NetworkAction {
    ...
    */
    ...
    public void onConnectionReady(String name, NetworkLogic conn) {
        ...
        publicKeys.add(i, k);
        //Потверждение подключения пользователя и отправка публичного ключа сервера и секретного ключа
    ...
    @Override
    public void getAllNameUser(String nameThis, NetworkLogic conn) {
        /*int p = connections.indexOf(conn);
        for(int i = 0; i < connections.size(); i++){
            connections.get(p).sendString("CodeGetAllName");
            //connections.get(p).sendString(idConn.get(i) + ": " + namesConn.get(i));
            connections.get(p).sendString(namesConn.get(i));
        }
        connections.get(p).sendString("CodeGetAllName");
        connections.get(p).sendString("StopGetAllNameUserSend");
        */
    }
